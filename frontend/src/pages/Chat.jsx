...
function Chat() {
    ...
    if (!user) {
        return <div className="chat-error"><h2>Please log in again.</h2></div>;
    }
    ...
    const searchUsers = useCallback(async (query) => {
        ...
        try {
            const res = await API.get(`/conversations/users?search=${encodeURIComponent(query)}`);
    ...
    const sendMessage = useCallback(async () => {
        ...
        try {
            const payload = { receiver_id: selectedUser.id, message: text };
            ...
            if (socketConnected.current) {
                socket.emit("send-message", { ...newMessage, conversation_id: selectedUser.id });
            }
    ...
    const handleTyping = useCallback((e) => {
        ...
        if (!typing && e.target.value.trim() && selectedUser) {
            setTyping(true);
            socket.emit("typing", { conversationId: selectedUser.id, userId: user.id });
        }
        clearTimeout(typingTimeoutRef.current);
        typingTimeoutRef.current = setTimeout(() => {
            setTyping(false);
            socket.emit("stop-typing", { conversationId: selectedUser?.id, userId: user.id });
        }, 2000);
    ...
    useEffect(() => {
        ...
        const onUserTyping = (data) => { if (data.userId !== user.id) setIsTyping(true); };
        const onUserStopTyping = (data) => { if (data.userId !== user.id) setIsTyping(false); };
            ...
            <div className="chat-sidebar">
                <div className="sidebar-header">
                    ...
                    <div className="sidebar-actions">
                        <button className="btn btn-secondary btn-sm" onClick={() => window.location.href = '/profile'} title="Profile">
                            <FaUser />
                        </button>
                        {isAdmin && (
                            <button className="btn btn-primary btn-sm" onClick={() => window.location.href = '/admin'}>
                                ⚙️ Admin
                            </button>
                        )}
                        <button className="btn btn-secondary btn-sm" onClick={() => setShowSearch(!showSearch)}>
                            <FaUserPlus />
                        </button>
                    ...
                    <div className="search-container animate-slide-in">
                        <div className="search-input-wrapper">
                            ...
                            <input
                                ...
                                autoFocus
                            />
                            <button className="search-close" onClick={() => { setShowSearch(false); setSearchQuery(""); setSearchResults([]); }}>
                                <FaTimes />
                            </button>
                        </div>
                            ...
                            <div className="search-results">
                                {searchResults.map((result) => (
                                    <div key={result.id} className="search-result-item" onClick={() => startConversation(result)}>
                                        <div className="avatar avatar-sm">{result.username?.charAt(0) || <FaUser />}</div>
                                        <div>
                                            <div className="search-result-name">{result.full_name}</div>
                                            <div className="search-result-username">@{result.username}</div>
                                        </div>
                                        <button className="btn btn-primary btn-sm"><FaUserPlus /></button>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="conversations-list">
                    {conversations.length === 0 ? (
                        ...
                        conversations.map((c) => {
                            const otherUser = c.user_one.id === user.id ? c.user_two : c.user_one;
                            const isOnline = onlineUsers.includes(otherUser.id);
                            const isActive = selectedUser?.id === otherUser.id;
                            return (
                                <div key={c.id} className={`conversation-item ${isActive ? 'active' : ''}`} onClick={() => loadMessages(otherUser)}>
                                    ...
                                    <div className="conversation-info">
                                        ...
                                        <div className="conversation-last-message">{c.last_message || "No messages yet"}</div>
                                    </div>
                ...
                {selectedUser ? (
                    <>
                        <ChatHeader
                            ...
                            onBack={() => { setSelectedUser(null); setMessages([]); setReplyTo(null); }}
                        />
                        ...
                        {replyTo && (
                            <div className="reply-preview">
                                ...
                                <button className="reply-preview-close" onClick={cancelReply}><FaTimes /></button>
                            </div>
                        )}

                        <div className="messages-container">
                            {loading ? (
                                <div className="loading-messages"><div className="loading-spinner"></div></div>
                            ) : (
                                <>
                                    {messages.length === 0 ? (
                                        <div className="empty-messages"><p>No messages yet. Say hello! 👋</p></div>
                                    ) : (
                                        messages.map((m) => (
                                                ...
                                                <div className="message-content">
                                                    {m.reply_to_message_id && (
                                                        <div className="message-reply-preview">
                                                            ...
                                                            <span className="reply-sender">{m.reply_to_sender_name || 'User'}</span>
                                                    ...
                                                    <div className="message-footer">
                                                        ...
                                                        <div className="message-actions">
                                                            <button className="message-action-btn" onClick={() => handleReply(m)} title="Reply"><FaReply /></button>
                                                            {m.sender_id === user.id && (
                                                                <>
                                                                    <button className="message-action-btn" title="Edit"><FaEdit /></button>
                                                                    <button className="message-action-btn" title="Delete"><FaTrash /></button>
                                                                </>
                                                            )}
                                                        </div>
                                                    </div>
                        ...
                        <div className="chat-input-container">
                            <button className="input-action"><FaImage /></button>
                            <button className="input-action"><FaSmile /></button>
                            <input
                                ref={inputRef}
                                value={text}
